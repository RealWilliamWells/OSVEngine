...
//        shader.setInt(("material." + name + number).c_str(), i); // TODO: Lighting and materials are not in shader code yet
    ...
    if (useLighting) {
        shader.setVec3("light.spotDir", light.spotDir);
        shader.setFloat("light.spotCutOff", light.spotCutOff);

        shader.setVec3("light.ambient", light.ambient); // TODO: handle ambience better when no presence of light exists
